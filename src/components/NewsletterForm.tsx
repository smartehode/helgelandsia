...
export function NewsletterForm() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const r = await fetch('/innsending/nyhetsbrev', { method: 'POST', body: new FormData(form) })
  ...
  if (status === 'sent') {
    return (
      <div className="rounded-2xl bg-sea/5 p-6 text-center ring-1 ring-sea/15">
        <p className="font-medium text-fjord">Takk! Nyhetsbrevet er sendt inn.</p>
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <div><label className={label}>Innhold *</label><textarea name="content" required rows={10} className={input} /></div>
