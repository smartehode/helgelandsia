...
export function BusinessForm() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const r = await fetch('/innsending/bedrift', { method: 'POST', body: new FormData(form) })
  ...
  if (status === 'sent') {
    return (
      <div className="rounded-2xl bg-sea/5 p-6 text-center ring-1 ring-sea/15">
        <p className="font-medium text-fjord">Takk! Bedriften er sendt inn.</p>
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <div><label className={label}>Bedriftsnavn *</label><input name="name" required className={input} /></div>
      <div><label className={label}>Slagord / kort beskrivelse</label><input name="tagline" className={input} /></div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div><label className={label}>E-post</label><input name="email" type="email" className={input} /></div>
      </div>
      <div><label className={label}>Nettside (URL)</label><input name="website" type="url" placeholder="https://…" className={input} /></div>
      <div><label className={label}>Adresse</label><input name="address" className={input} /></div>
      <div><label className={label}>Logo (maks 8 MB)</label><input name="logo" type="file" accept="image/*" className="block w-full text-sm text-muted file:mr-3 file:rounded-full file:border-0 file:bg-sea file:px-4 file:py-2 file:text-sm file:font-medium file:text-white hover:file:bg-fjord" /></div>
