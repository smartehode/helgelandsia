...
export async function POST(req: Request) {
  ...
  if (!title || !content) {
    return Response.json({ error: 'Tittel og brødtekst må fylles ut.' }, { status: 400 })
  }
  ...
  try {
    const doc = await payload.create({
      ...
      data: {
        ...
        excerpt: String(fd.get('excerpt') || '').slice(0, 300) || undefined,
    ...
    return Response.json({ error: 'Kunne ikke lagre artikkelen.' }, { status: 500 })
  }
}
