...
const RSS_URL = 'https://hemnes.kommune.no/rss/'

// Generell kommunefeed — keyword-filter er viktig her.
const KEYWORDS =
...
export async function fetchHemnes(): Promise<Kunngjoring[]> {
  try {
    ...
    for (const item of items) {
      ...
      result.push({ kommune: 'Hemnes', tittel, dato, url, kilde: 'Hemnes kommune' })
    }
    return result
  } catch {
    return []
